...
from ..streamless import Streamless
...
class TestIteration(TestCase):
    ...
    def test_iterates_container(self):
        n = randint(1, 10)
        self.assertEqual(list(Streamless(range(n))), list(range(n)))
        self.assertEqual(list(Streamless(list(range(n)))), list(range(n)))
        self.assertEqual(list(Streamless(tuple(range(n)))), list(range(n)))
        self.assertEqual(list(Streamless({n: 'foo'})), [n])
        self.assertEqual(list(Streamless('abc')), list('abc'))

    def test_iterates_empty_container(self):
        self.assertEqual(list(Streamless(range(0))), [])
        self.assertEqual(list(Streamless([])), [])
        self.assertEqual(list(Streamless(tuple())), [])
        self.assertEqual(list(Streamless({})), [])
        self.assertEqual(list(Streamless('')), [])
    ...
    def test_iterates_sequence(self):
        ...
        self.assertEqual(list(Streamless(generate())), list(range(n)))

    def test_iterates_range(self):
        self.assertEqual(list(Streamless(range(3))), [0, 1, 2])

    def test_does_not_iterate_non_sequence(self):
        self.assertRaises(TypeError, list, Streamless(None))
        self.assertRaises(TypeError, list, Streamless(10))

    def test_evaluates_lazily(self):
        iterations = []
        sequence = iter(Streamless(record(range(5), iterations)))
...
class TestList(TestCase):
    ...
    def test_returns_empty_for_empty_stream(self):
        self.assertEqual(Streamless().list(), [])

    def test_returns_items_as_list(self):
        self.assertEqual(Streamless(range(3)).list(), [0, 1, 2])
    ...
    def test_turns_iterator_into_list(self):
        ...
        self.assertEqual(Streamless(iterate()).list(), [item])


class TestTuple(TestCase):
    """Tests for `tuple`."""
    def test_returns_empty_for_empty_stream(self):
        self.assertEqual(Streamless().tuple(), tuple())

    def test_returns_items_as_tuple(self):
        self.assertEqual(Streamless(range(3)).tuple(), (0, 1, 2))
...
class TestNegate(TestCase):
    """Tests for `negate`."""
    def test_returns_Streamless(self):
        self.assertIsInstance(Streamless([1]).negate(), Streamless)

    def test_does_nothing_if_empty(self):
        self.assertEqual(Streamless().negate().list(), [])

    def test_negates_true_items_to_False(self):
        trues = [True, 1, 'y', [0], {0: 0}]
        self.assertEqual(Streamless(trues).negate().list(), [False] * len(trues))

    def test_negates_false_items_to_True(self):
        falses = [False, 0, '', [], {}]
        self.assertEqual(Streamless(falses).negate().list(), [True] * len(falses))

    def test_consumes_lazily(self):
        iterations = []
        Streamless(record([1, 2], iterations)).negate()
        self.assertEqual(iterations, [])
...
class TestCount(TestCase):
    ...
    def test_returns_zero_if_empty(self):
        self.assertEqual(Streamless().count(), 0)

    def test_counts_items(self):
        values = [randint(0, 10) for _ in range(randint(0, 5))]
        self.assertEqual(Streamless(values).count(), len(values))

    def test_counts_even_false_items(self):
        self.assertEqual(Streamless([None]).count(), 1)


class TestEmpty(TestCase):
    """Tests for `empty`."""
    def test_True_if_empty(self):
        self.assertTrue(Streamless().empty())
        self.assertTrue(Streamless([]).empty())
        self.assertTrue(Streamless(range(0)).empty())

    def test_False_if_nonempty(self):
        self.assertFalse(Streamless([False]).empty())
        self.assertFalse(Streamless(range(3)).empty())

    def test_consumes_only_one_item(self):
        iterations = []
        empty = Streamless(record(range(3), iterations)).empty()
        self.assertFalse(empty)
        self.assertEqual(iterations, [0])

    def test_works_for_iterator(self):
        self.assertTrue(Streamless(iter([])).empty())
...
class TestForEach(TestCase):
    ...
    def test_calls_function_on_each_item(self):
        ...
        Streamless(inputs).for_each(process)
    ...
    def test_adds_kwargs(self):
        ...
        n = randint(1, 3)
        Streamless(range(n)).for_each(process, kwargs={'kwarg': 'foo'})
    ...
    def test_stops_at_exception(self):
        ...
        stream = Streamless(record([1, 2, 3, 4, 5], iterations))
...
class TestDrain(TestCase):
    ...
    def test_consumes_all_items(self):
        ...
        Streamless(record(inputs, iterations)).drain()
...
class TestFilter(TestCase):
    ...
    def test_adds_kwargs(self):
        ...
        length = 5
        stream = Streamless(range(length))
...
class TestFilterOut(TestCase):
    ...
    def test_filters_on_identity_by_default(self):
        ...
        self.assertEqual(
            Streamless(values).filter_out().list(),
            [item for item in values if not item])

    def test_filters_on_criterion(self):
        even = lambda item: item % 2 == 0
        self.assertEqual(
            Streamless([0, 1, 2, 3, 4]).filter_out(even).list(),
            [1, 3])
...
class TestMap(TestCase):
    ...
    def test_applies_function(self):
        ...
        self.assertEqual(
            Streamless(range(3)).map(double).list(),
            [0, 2, 4])

    def test_adds_kwargs(self):
        stream = Streamless([[1, 2, 3, 4]])
        self.assertEqual(
...
class TestLimit(TestCase):
    ...
    def test_stops_iteration_at_limit(self):
        self.assertEqual(
            Streamless(range(5)).limit(2).list(),
            [0, 1])

    def test_does_nothing_if_limit_is_beyond_stream(self):
        self.assertEqual(
            Streamless(range(3)).limit(10).list(),
            [0, 1, 2])

    def test_does_not_consume_beyond_limit(self):
        iterations = []
        Streamless(record(range(5), iterations)).limit(2).drain()
        self.assertEqual(iterations, [0, 1])
...
class TestUntilValue(TestCase):
    ...
    def test_stops_at_sentinel(self):
        self.assertEqual(Streamless(range(5)).until_value(3).list(), [0, 1, 2])

    def test_stops_normally_if_sentinel_not_found(self):
        self.assertEqual(
            Streamless(range(5)).until_value(9).list(),
            [0, 1, 2, 3, 4])

    def test_checks_value_not_identity(self):
        self.assertEqual(
            Streamless([[0], [1], [2], [3]]).until_value([2]).list(),
            [[0], [1]])

    def test_does_not_consume_beyond_sentinel(self):
        iterations = []
        Streamless(record(range(5), iterations)).until_value(2).drain()
...
class TestUntilIdentity(TestCase):
    ...
    def test_stops_at_sentinel(self):
        stop = object()
        self.assertEqual(
            Streamless([0, 1, stop, 3]).until_identity(stop).list(),
            [0, 1])

    def test_stops_normally_if_sentinel_not_found(self):
        self.assertEqual(
            Streamless(range(5)).until_identity(9).list(),
            [0, 1, 2, 3, 4])

    def test_checks_identity_not_value(self):
        self.assertEqual(
            Streamless([[0], [1], [2], [3]]).until_identity([2]).list(),
            [[0], [1], [2], [3]])

    def test_does_not_consume_beyond_sentinel(self):
        stop = object()
        iterations = []
        stream = Streamless(record([0, 1, stop, 2], iterations))
...
class TestUntilTrue(TestCase):
    ...
    def test_stops_when_condition_met(self):
        ...
        self.assertEqual(
            Streamless([0, 1, 2, 3]).until_true(many).list(),
            [0, 1])

    def test_stops_normally_if_condition_never_met(self):
        huge = lambda item: item > 10
        self.assertEqual(
            Streamless(range(5)).until_true(huge).list(),
            [0, 1, 2, 3, 4])

    def test_does_not_consume_beyond_sentinel(self):
        iterations = []
        many = lambda item: item > 1
        stream = Streamless(record(range(5), iterations))
    ...
    def test_adds_kwargs(self):
        ...
        arg = randint(0, 10)
        length = randint(1, 5)
        Streamless(range(length)).until_true(check, kwargs={'kwarg': arg}).drain()
...
class TestWhileTrue(TestCase):
    ...
    def test_stops_when_condition_no_longer_met(self):
        ...
        self.assertEqual(
            Streamless([0, 1, 2, 3]).while_true(few).list(),
            [0, 1])

    def test_stops_normally_if_condition_never_met(self):
        reasonable = lambda item: item < 10
        self.assertEqual(
            Streamless(range(5)).while_true(reasonable).list(),
            [0, 1, 2, 3, 4])
    ...
    def test_adds_kwargs(self):
        ...
        arg = randint(0, 10)
        length = randint(1, 5)
        Streamless(range(length)).while_true(check, kwargs={'kwarg': arg}).drain()
...
class TestConcat(TestCase):
    """Tests for `concat`."""
    def test_returns_empty_for_empty_stream(self):
        self.assertEqual(Streamless().concat().list(), [])
    ...
    def test_concatenates_lists(self):
        self.assertEqual(
            Streamless([[1, 2], [3, 4]]).concat().list(),
            [1, 2, 3, 4])
...
class TestPartition(TestCase):
    ...
    def test_preserves_order(self):
        ...
        self.assertEqual(
            Streamless(inputs).partition(mod_3),
            {
                0: [0, 3, 6, 9],
                1: [4, 7, 1],
                2: [8, 2, 5],
    ...
    def test_preserves_duplicates(self):
        ...
        inputs = [0, 1, 0, 3, 0, 5]
        self.assertEqual(
            Streamless(inputs).partition(even),
            {
                True: [0, 0, 0],
                False: [1, 3, 5],
            })
    ...
    def test_adds_key_kwargs(self):
        ...
        self.assertEqual(
            Streamless(range(3)).partition(multiply, key_kwargs={'factor': 2}),
            {
                0: [0],
                1 * factor: [1],
                2 * factor: [2],
